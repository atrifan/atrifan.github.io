...
function generateServerToolsSQL(): string {
  ...
  lines.push('DO $$');
  lines.push('DECLARE');
  lines.push('  v_api_key_id UUID;');
  lines.push('  v_tool_id UUID;');
  lines.push('  v_tool_name TEXT;');
  lines.push('BEGIN');
  lines.push("  SELECT id INTO v_api_key_id FROM api_keys");
  lines.push("  WHERE user_id = 'user_37inOsUBpoqj1Nv5ZyeZ7rBOUKo' AND server_name = 'default';");
  lines.push('');
  lines.push('  IF v_api_key_id IS NOT NULL THEN');
  lines.push("    FOR v_tool_id, v_tool_name IN SELECT id, name FROM tools WHERE tool_type = 'NATIVE'");
  lines.push('    LOOP');
  lines.push('      INSERT INTO server_tools (api_key_id, tool_id, is_enabled)');
  lines.push('      VALUES (v_api_key_id, v_tool_id, true)');
  lines.push('      ON CONFLICT (api_key_id, tool_id) DO NOTHING;');
  lines.push('    END LOOP;');
  lines.push('  END IF;');
  lines.push('END $$;');
